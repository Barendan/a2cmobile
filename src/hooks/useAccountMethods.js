...
const useAccountMethods = () => {
  ...
  const onValidateMemberInfo = () => {
    ...
    const formatDate = date => {
      ...
      const properDate = a => (a.length < 2 ? 0 + a : a);

      return properDate(month) + '/' + properDate(day) + '/' + year;
    };

    setErrorMessage('');
    setMemberRecord(null);
    setLoading(true);
    MemberService.validateMemberInfo(memberID, formatDate(dateOfBirth), zipcode)
      .then(data => {
        setLoading(false);
  ...
  React.useEffect(() => {
    ...
    updateLoginMethod(
      'loginType',
      selectedIndex === 0 ? 'email' : 'phone_number',
    );
  ...
  React.useEffect(() => {
    ...
    if (loginMethod.loginType === 'na') {
      ...
      setDisableNextButton(
        loginMethod.loginType.length === 0 ||
          (loginMethod.loginType === 'email'
            ? !HelperMethods.isValidEmail(loginMethod.login)
            : loginMethod.login.length < 9),
      );
    }
